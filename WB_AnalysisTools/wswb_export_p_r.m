...
% plot_p_compare(wy_PP_PV(:,3),wy_PP_PV(:,2),wy_PP_PV(:,1),figname)
% 
% 
% %% VIC SCALING INTERVAL: 1961-1990
% % per VIC methodology, VIC pixels are scaled by mean PRISM for interval
% % 1961-1990, so comparison of the two should probably be based on this time
% intvl = 1961:1990;
% wy_PP_PV_intvl = wy_PP_PV(ismember(wy_PP_PV(:,1),intvl),:);
% figname = fullfile(['PDATA_','VIC'], 'VIC_vs_PRISM_1961-1990');
% plot_p_compare(wy_PP_PV_intvl(:,3),wy_PP_PV_intvl(:,2),wy_PP_PV_intvl(:,1),figname)

%% MEAN PET FOR WS
pet = st_ws.WYtot.PET.CIMIS.mo_cy.Oct1.data;
pet = mean(pet);
csvwrite('pet_cimis_mean.csv',pet)
...
% p-value
[~,pv] = corrcoef(p1,p2); pval = pv(2);			
if pval < 0.001, txt_pval = 'pval<0.001';
else txt_pval = ['pval=',num2str(pval,3)];
end
x1 = [min(p1),max(p1)];
line(x1,polyval(pfit_p1p2,x1),'color','b')
title(['Precip: ',num2str(min(t)),'-',num2str(max(t)),', y = ',num2str(pfit_p1p2(1),3),'*x + ',num2str(round(pfit_p1p2(2))),', ',txt_pval,'   '])
...
% p-value
[~,pv] = corrcoef(t,dP); pval = pv(2);			
if pval < 0.001, txt_pval = 'pval<0.001';
else txt_pval = ['pval=',num2str(pval,3)];
end
x2 = [min(t),max(t)];
line(x2,polyval(pfit_dP,x2),'color','b')
title(['Slope=',num2str(pfit_dP(1),3),', ',txt_pval,'  '])
ylabel('Residual: PRISM-LinFit (mm)')
xlabel('WY')
axis tight
