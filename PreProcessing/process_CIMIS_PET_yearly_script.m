...
for yy = 1:length(yrs)
    ...
    for mm = 1:12
        mo = mo_str{mm};
        ndays = daysinmonth(mm,yr); % calc days in mo for nonleap year
        try
            [Z,refmat] = arcgridread(['CIMIS_ETo_',yr_txt,'_',mo,'.asc']);
        catch
            Z = nan(550,500);
        end
        Ztot(:,:,mm) = Z.*ndays;
    ...
    fname = ['ST_CIMIS_PET_CY_',yr_txt,'.mat'];
    save(fname,'st_cimis_pet')
    display([num2str(yy/length(yrs)*100),'% done'])
end %for yy
